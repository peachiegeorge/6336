function X = forwardEulerFlipTheta(P, eval_f, x_start, p, eval_u, t_start, t_stop, timestep)
...
for n=1:numTSteps
    ...
    if mod(n, numTSteps/t_stop) == 0
        disp("At time t = " + num2str(t(n+1)));
    end    
    ...
    XMat = convertSeirCellToMat(X(:,n+1));
    XClamp = max(XMat, 0); % Clamp to 0
    X(:,n+1) = convertSeirMatToCell(XClamp);
