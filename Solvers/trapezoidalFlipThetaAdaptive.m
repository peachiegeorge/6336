...
function [x, t, newtonLastIter] = trapezoidalFlipThetaAdaptive(P, x_start, p, u, t_stop, timestep)
dt = timestep;
maxT = t_stop;
x0SeirCell = x_start;
x(:,1) = cell2vec(x0SeirCell,1);

[p_day, p_night] = flipThetaSign(P, p);
...
while prevT < maxT
	...
	if(tStep > 1)
		...
		%dt must not make the next t pass the middle of each day
		if mod(t(tStep-1), 1) < 0.5
			maxCurT = floor(t(tStep-1)) + 0.5;
		else
			maxCurT = ceil(t(tStep-1));
		end
